...
export default function ContributionFeed({
  ...
  return (
    <div className="flex flex-col gap-3 overflow-y-auto max-h-[420px] pr-1">
      <p
        className="text-[10px] font-bold tracking-widest uppercase sticky top-0 font-[family-name:var(--font-space-mono)]"
        style={{
      ...
      {entries.length === 0 ? (
        ...
        entries.map(([pid, text]) => {
          ...
          return (
            ...
            >
              <p
                className="text-[10px] font-[family-name:var(--font-space-mono)] tracking-widest mb-2"
                style={{
                  color: isLatest ? "var(--lab-accent)" : "var(--lab-text-dim)",
                }}
              >
                {playerNames[pid] ?? pid}
              </p>
